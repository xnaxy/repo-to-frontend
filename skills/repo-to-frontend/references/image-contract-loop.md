...
- contract：`scope:'backend-product',sourceEvidence:[文件证据],pages:[{id}],capabilities:[...]`。page id包含唯一页面/状态标识。visible能力为 `{id,presentation:'visible',requirements:[{id,pageId,kind,expected}]}`，kind为action/data/state/result，expected具体写字段、单位、权限或状态。internal能力为 `{id,presentation:'internal',representedBy:[可见能力id],reason}`。另保留原始取证台账；JSON不替代独立源码覆盖审查。
